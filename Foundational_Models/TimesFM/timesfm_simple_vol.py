...
results_dir = os.path.join(os.path.dirname(__file__), 'simple_vol_results')
...
def preprocess_volatility_data(df, expiry, window):
    """
    Preprocess data using only volatility feature
    ...
    """
    # Use the volatility column for the specific expiry
    vol_column = f'{pred_value_to_char(expiry)}_vol'
    
    if vol_column not in df.columns:
        raise ValueError(f"Volatility column {vol_column} not found in data")
    
    # Extract volatility data
    volatility_data = df[vol_column].values
    
    # Remove rows with NaN values
    valid_indices = ~np.isnan(volatility_data)
    volatility_data = volatility_data[valid_indices]
    
    # Create sequences
    X, y = [], []
    for i in range(window, len(volatility_data) - expiry + 1):
        X.append(volatility_data[i-window:i])
        y.append(volatility_data[i+expiry-1])
...
def evaluate_timesfm_simple(model, X_test, y_test, y_scaler, device):
    """
    Evaluate TimesFM model using only volatility feature
...
def plot_predictions_using_line_plot(y_true, y_pred, expiry, save_path):
    ...
    ax, fig = line_plot(time_index, y_pred, ylabel='pred_vol', 
                        graphtitle=f'TimesFM Simple: True vs Predicted {pred_value_to_char(expiry)}_vol', 
...
def run_timesfm_simple_experiment(expiry):
    """
    Run TimesFM experiment using only volatility feature
    ...
    print(f"\nRunning TimesFM Simple experiment for {pred_value_to_char(expiry)} expiry")
    print(f"Using only volatility feature: {pred_value_to_char(expiry)}_vol")
    print(f"Window size: {window}")
    
    # Preprocess data using only volatility
    X_raw, y_raw, y_scaler = preprocess_volatility_data(
        df, expiry, window
    )
    ...
    metrics, predictions, y_test_inv = evaluate_timesfm_simple(
        model, X_test, y_test, y_scaler, device
    )
    
    # Create and save prediction plot using line_plot
    plot_path = os.path.join(plots_dir, f"{expiry}_vol_vs_true_test_TimesFM_simple.png")
...
def main():
    """Main function to run TimesFM simple experiments"""
    print("Starting TimesFM Simple Experiments (Volatility Only)")
    print("=" * 60)
    ...
    for expiry in [5, 22, 66, 252]:
        ...
        try:
            metrics = run_timesfm_simple_experiment(expiry=expiry)
    ...
    if all_metrics:
        ...
        if metrics_data:
            ...
            metrics_csv_path = os.path.join(metrics_dir, 'metrics_simple_vol.csv')
    ...
    print("\n" + "=" * 60)
    print("All TimesFM Simple experiments completed!")
