...
# Create LSTM_all_c5_results directory structure
main_results_dir = os.path.join(os.path.dirname(__file__), 'LSTM_all_c5_results')
...
def LSTM_all_c5_main_experiment(expiry, features_names, features_names_suffix, seed=42):
    # Set seed for reproducibility
    set_seed(seed)
    
    window = 5  # Fixed context window of 5 instead of expiry*2
    batch_size = 128
...
for expiry in [5, 22, 66, 252]:
    ...
    for i, (features, name) in enumerate(feature_combinations):
        ...
        for run_idx, seed in enumerate(seeds):
            ...
            metrics = LSTM_all_c5_main_experiment(
                expiry=expiry, 
                features_names=features, 
                features_names_suffix=f'LSTM_all_c5_{name}_{pred_value_to_char(expiry)}_exp_run{run_idx+1}_seed{seed}',
                seed=seed
            )
    ...
    # Calculate statistics across the 3 runs for each configuration
