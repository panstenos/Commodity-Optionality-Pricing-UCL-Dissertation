...
# Create GRU_all_results directory structure
main_results_dir = os.path.join(os.path.dirname(__file__), 'GRU_all_results')
...
def GRU_all_main_experiment(expiry, features_names, features_names_suffix, seed=42):
    # Set seed for reproducibility
    set_seed(seed)
...
for expiry in [5, 22, 66, 252]:
    ...
    for i, (features, name) in enumerate(feature_combinations):
        ...
        for run_idx, seed in enumerate(seeds):
            ...
            metrics = GRU_all_main_experiment(
                expiry=expiry, 
                features_names=features, 
                features_names_suffix=f'GRU_all_{name}_{pred_value_to_char(expiry)}_exp_run{run_idx+1}_seed{seed}',
                seed=seed
            )
