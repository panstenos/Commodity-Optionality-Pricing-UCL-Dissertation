...
def calculate_fair_metrics(df, predictions_file="volatility_predictions/avg_pred_volatilities.json", output_file="garch_forecast_metrics_fair.csv"):
    """
    ...
    for expiry_date, vals in avg_pred_volatilities_test.items():
        ...
        for metric in [mape, mae, mse, rmse, mase]:
            ...
            vals_subset = vals[-488+pred_char_to_value(expiry_date):]
...
def generate_volatility_plots(df, predictions_file="volatility_predictions/avg_pred_volatilities.json", plot_dir="volatility_plots"):
    ...
    for expiry_date, vals in avg_pred_volatilities_test.items():
        ...
        ax, fig = line_plot(
            df['date'][pred_char_to_value(expiry_date) * 2:], 
            vals, 
...
def generate_fair_volatility_plots(df, predictions_file="volatility_predictions/avg_pred_volatilities.json", plot_dir="volatility_plots"):
    ...
    for expiry_date, vals in avg_pred_volatilities_test.items():
        ...
        ax, fig = line_plot(
            df['date'][-488+pred_char_to_value(expiry_date):], 
            vals[-488+pred_char_to_value(expiry_date):], 
            f'vol_pred_avg, horizon={pred_char_to_value(expiry_date)}', 
            graphtitle=f'{expiry_date}_vol', 
            show=False
        )
        _, _ = line_plot(
            df['date'][-488+pred_char_to_value(expiry_date):], 
            df[f'{expiry_date}_vol'][-488+pred_char_to_value(expiry_date):], 
            'vol_true', 
            linecolor='red', 
            ax=ax, 
            show=False
        ...
        ax, fig = line_plot(
            df['date'][-488+pred_char_to_value(expiry_date):], 
            df[f'al_lme_prices_log_returns'][-488+pred_char_to_value(expiry_date):], 
            'log_returns', 
            graphtitle=f'{expiry_date}_vol', 
            linecolor='red', 
            show=False
...
def run_complete_garch_analysis(data_path='../Data/aluminium_pre_inputs.csv'):
    ...
    plot_raw_data(df['date'], df['al_lme_prices_log_returns'], 'al_lme_prices_log_returns')
